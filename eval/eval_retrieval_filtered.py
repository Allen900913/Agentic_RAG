"""
eval_retrieval_filtered.py — With-filter vs Without-filter Retrieval Comparison

Re-runs the hybrid retrieval lane from eval_retrieval.py twice per query:
  1. "nofilter" — same as eval_retrieval.py's hybrid mode (no payload filter)
  2. "filter"   — applies rag_query.py's parse_query_filters() +
                  build_qdrant_filter() as a hard payload filter on both
                  Prefetch lanes before RRF fusion (mirrors retrieve()'s
                  step 0/2, including the "0 candidates -> retry unfiltered"
                  fallback).

Only meaningful against `us_stock_rag` (built by data_update.py), since that
is the only collection whose payload carries filing_type/fiscal_year/
fiscal_period — `us_stock_rag_unstructured` has no such fields, so every
filter there would be a no-op.

Usage:
  python eval/eval_retrieval_filtered.py
  python eval/eval_retrieval_filtered.py --eval-set eval/eval_set.json --output eval/results_filtered.json
"""
...
from rag_query import build_qdrant_filter, QUERY_FILTER_SYSTEM_PROMPT, FILTERABLE_FIELDS  # noqa: E402

from eval.eval_retrieval import (  # noqa: E402
    collect_all_sources, expand_relevant, unique_sources_in_order,
...
def hybrid_retrieve_filtered(client, q_dense, q_sparse_indices, q_sparse_values, limit, query_filter):
    from qdrant_client import models

    def _run(active_filter):
        return client.query_points(
            collection_name=COLLECTION_NAME,
            prefetch=[
                models.Prefetch(
                    query=q_dense, using=DENSE_VECTOR_NAME,
                    limit=DEFAULT_FETCH, filter=active_filter,
                ),
                models.Prefetch(
...
def evaluate(eval_set: dict, client, bge_m3, groq_client, ks: list[int]) -> dict:
    ...
    for q in queries:
        ...
        # ── Detect hard filter via LLM (same as rag_query.py retrieve() step 0) ──
        detected_filters = parse_query_filters_groq(query_str, groq_client)
        query_filter = build_qdrant_filter(detected_filters)
        ...
        nofilter_pts = hybrid_retrieve_filtered(client, q_dense, s_idx, s_val, chunk_limit, None)
        filter_pts   = hybrid_retrieve_filtered(client, q_dense, s_idx, s_val, chunk_limit, query_filter)

        per_mode_sources = {
            "nofilter": unique_sources_in_order(nofilter_pts),
            "filter":   unique_sources_in_order(filter_pts),
        }
...
def print_table(summary: dict, ks: list[int]) -> None:
    ...
    width = 78
    print(f"\n{'=' * width}")
    print("HARD-FILTER COMPARISON — Hybrid retrieval, no-filter vs LLM-detected filter")
    ...
    for cat in cats:
        ...
        header = f"    {'Metric':<10}{'nofilter':>12}{'filter':>12}    Delta"
        print(header)
        print(f"    {'-' * 10}" + ("-" * 12) * 2 + "    ------")
        for m in metric_keys:
            row = f"    {label(m):<10}"
            v0 = summary["nofilter"][cat].get(m, 0.0)
            v1 = summary["filter"][cat].get(m, 0.0)
            row += f"{fmt_pct(v0):>12}{fmt_pct(v1):>12}"
            d = v1 - v0
            sign = "+" if d >= 0 else ""
            row += f"   {sign}{d*100:+.1f}pp"
            print(row)
        print()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Hard-filter vs no-filter retrieval comparison (hybrid only)",
    )
    parser.add_argument("--eval-set", default="eval/eval_set.json")
    parser.add_argument("--output", default="eval/results_filtered.json")
