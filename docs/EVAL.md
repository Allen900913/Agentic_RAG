...
2. **這不是控制變因的 A/B。** query 前處理模型不同（單發 `gpt-oss-120b`／agentic `gpt-oss-20b`），且 agentic 天生檢索多個子問題 → contexts 更多，**機械性地有利於 recall／relevance 類指標**。要歸因到「agentic 架構本身」還需要把這兩項對齊。
