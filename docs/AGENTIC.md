...
- [A8 2026-08-15 live web 的可重現評測：把不確定性切開，而不是硬定 gold](#a8-2026-08-15-live-web-的可重現評測把不確定性切開而不是硬定-gold)
- [A9 2026-08-15 期別稽核：答案自稱「最新一季」，卻引用了較舊的期別](#a9-2026-08-15-期別稽核答案自稱最新一季卻引用了較舊的期別)
...
## A8 2026-08-15 live web 的可重現評測：把不確定性切開，而不是硬定 gold

**問題**：live web 沒辦法定 gold（資料每天在變），所以 A6／A7 做的整套機制——白名單＋本地複核、URL 抽發布日、時效分層、去重、單域名上限、query 級預算、`kb_unfixable`、system message 放行條款——在 eval 裡**執行次數是 0**（snapshot 恆關），唯一的證據是 n=1 實跑軼事，而 A6 自己就寫過「n=1 測不出這條路的任何東西」。

### 切法：確定性的與含 LLM 的分開，各用各的量尺

| 部分 | 用什麼測 | 為什麼 |
|---|---|---|
| 白名單／去重／單域名上限／日期算術／預算／`kb_unfixable` | [`verify_web_gate_isolation.py`](../eval/verify_web_gate_isolation.py) 六道閘門 79 項斷言 | **零 LLM、零網路、零 Qdrant、秒級**。已經有了，不重測 |
| Generator 有沒有真的引用 web 數字、衝突時有沒有並陳並標時點、web 有沒有被無差別觸發 | [`web_replay.py`](../web_replay.py) fixture ＋ [`check_web_claims.py`](../eval/check_web_claims.py) 逐條斷言 | **必須含 LLM**，所以必須固定輸入 |

「VCR 只為了測預算／白名單有沒有生效」是**重複投資**——那些已經有更便宜的測法。fixture 的價值在**解鎖含 LLM 的端到端路徑**，這是唯一新增的能力。

### 為什麼不用 RAGAS 而用逐條斷言

這條路只有 5 題。本專案 RAGAS 的 MDE 在 n=100 是 0.067，n=5~10 換算約 **0.17~0.21**——任何分數都落在噪音裡、解讀不了。判準改成零噪音的逐條斷言（同 [`check_number_defects.py`](../eval/check_number_defects.py)），PASS／FAIL／**N-A** 三態。

### 四個設計上的坑（每一個都會讓 fixture 失效而不自知）

**① 錄的層級**。必須錄 `TavilyClient.search()` 的**原始回應**，不能錄 `_tavily_search()` 的回傳字串——後者是跑完 `_host_allowed` 複核 → `_normalize_url` 去重 → 單域名上限 → `_url_published_date` 抽日期 → 過時過濾 → 摘要截斷之後的成品。錄在那裡等於把要測的六道一起 mock 掉。同 `llm_replay` 刻意 key 在函式輸入輸出的理由。
> 驗證方式：拿一份人工 fixture 塞進地區子網域＋非白名單主機＋amp 變體，確認下游照常濾掉（實測 5 筆原始 → 保留 1 筆）。

**② `get()` 與 `put()` 兩邊都要 deepcopy**。`_dedupe_web_results` 就地寫 `r["_pub_date"]`（`agentic_rag_v2.py:584`），而我們錄的正是那些 dict。
- `put()` 少 → 快取被塞進 `date` 物件 → 落盤 `TypeError`。首次錄製**安靜掉了第 8 筆**：atexit 的例外只印 traceback、**不影響 exit code**，看起來成功。
- `get()` 少 → **更嚴重**：同一 key 重放第二次時日期已被上一次塞好 → `_url_published_date` 那段**根本不執行**，兩次重放走不同程式路徑，fixture 就不再是「固定輸入」。
- 連帶：序列化失敗要在 `put()` 當場炸（`RecordError`），不能拖到 atexit；且必須是**獨立型別並由呼叫端 re-raise**，否則會被 `_tavily_search` 的優雅降級吞成「web 查無結果」（同 `FixtureMiss`）。

**③ 只錄 web 不夠，要連 `llm_replay` 一起錄**。pipeline 送給 Tavily 的 query 是 Planner／Grader 的 LLM 輸出決定的，而 MoE 溫度 0 不固定專家路由（plan 實測 48% 重跑不同）→ 重放時會生出不同的 web query、直接 fixture miss。

**④ 斷言不能寫死成某一次錄製的答案**。錄製時 LLM 引 stockanalysis 的 `$4.46 兆`、重放時引 finance.yahoo 的 `$4.45 兆`——**兩個都在 fixture 裡、都正確**。fixture 是一個**值空間**，不是單一答案。斷言只能測「不論它挑哪個都必須成立」的性質：
- 有沒有引用**任何** web 來源（不指定哪個）
- 引用的主機是否都在白名單內、**是否含地區子網域**（fixture 原始回應裡確實有 `ca.`／`sg.`／`nz.finance.yahoo.com`，所以這條測得到真防線）
- **答案裡每一個 `ddd.dd` 報價都必須在 fixture 原始內容裡逐字出現**（＝零幻覺斷言，這是整組裡最強的一條）
- KB 舊值與其時點是否與 web 新值並陳（`$4,342.02B` ＋ `2026-06-12`）

附帶：驗收腳本第一版**自己誤報三題 FAIL**，因為正則只認 ASCII `[web:]` 而 `gpt-oss-120b` 吐的是全形 `【web:】`（A5 早有記載）。

### 驗收與判別力

兩次**獨立取樣**（錄製那輪、重放那輪，LLM 挑了不同來源與不同事實）跑同一組斷言都 **5/5 PASS**。照「全綠先當壞消息查」的紀律，注入六種變異確認抓得到：拔掉 web 引用／換成地區子網域／陰性對照被觸發／拿掉 KB 舊值日期／股價全部竄改／**只竄改其中一個股價** → **6/6 全被抓到**。
> 最後那一種是加上零幻覺斷言之後才抓得到的：原本只要求「有一個價格形狀的數字」，改了一個仍有其他正確值留著就漏。

⚠ **「斷言零噪音」不等於「結果零噪音」。** 斷言本身確實是確定性的——同一份結果檔跑幾次都一樣。但**產生結果檔的那一步有 LLM 取樣變異**：實測四次取樣，`web-02` 的答案每次挑的數字、句子結構都不同。所以 `5/5 PASS` 的正確讀法是「**這一次取樣**沒問題」，不是「這個系統不會出問題」；要提升信心只能多取樣，重跑檢查器沒有用。

⚠ **而且斷言的 FAIL 也可能是量尺錯。** 第四次取樣 `web-02` FAIL，說「2026 年的平均股價為 $196.82」是憑空生成——**是誤報**：fixture 的 macrotrends 表格裡就有 `| 2026 | 196.8165 | 188.6200 | 235.4660 | 164.9780 | 224.0900 |`，答案是**正確地四捨五入**。`numbers_must_be_in_fixture` 當時只做逐字比對，來源的小數位數常多於答案（同一種形狀還有 `P/E Ratio Trailing: 31.325687` → 答案寫 `31.33`）。已補上捨入容忍。
> 我拿那個 FAIL 當真陽性，寫了一整條「reflect 引入幻覺」的 BACKLOG、一段 CHANGELOG、一節 docs，才在實作生產版檢查時發現來源裡有 `196.8165`。**CLAUDE.md 的「稽核回傳 0 筆問題先當壞消息查」需要一個鏡像條款：稽核回報 FAIL，也要先問「是不是量尺錯」再問「系統怎麼壞的」。** 兩個方向的成本一樣低（一次 `grep`），代價卻是幾份文件的錯誤內容。

### 量到的兩件事

**`web-01` 的正確行為不是 validator 做到的。** KB 舊值（`$4,342.02B` @ 2026-06-12）與 web 新值並陳並各自標時點——這正是 `find_authority_conflicts` R3「財報優先」會判反的情境，而它**根本沒被觸發**（web 內容不在 `chunks` 裡，見 [`BACKLOG.md`](../BACKLOG.md)）。是 Generator 靠 prompt 做到的。**那條防線當時是空的，只是剛好沒出事**——2026-08-19 已補上 **R4 `find_unreconciled_web_conflicts`**（見下方〈R4〉）。⚠ 補的方式**不是**擴充 R3：R3 判「一邊為錯」，套到 web 會系統性報舊數字。

**`web-03` 是真缺口，但不是這裡寫的那個。** ⚠ 本節第一版寫「KB 裡 MSFT 最新 10-Q 是 `202603`，5 個月前的資料被當成最新一季，成因是 `realtime_need` 判 `none` 讓時效改判不觸發」——**診斷錯了**。MSFT 在 KB 的天花板是 `MSFT_10K_2026`（FY2026，涵蓋到 2026-06-30），距 as-of 只有 46 天，KB 不缺資料、web 也不該觸發。真正的缺陷與修法見 [A9](#a9-2026-08-15-期別稽核答案自稱最新一季卻引用了較舊的期別)。

---

## A9 2026-08-15 期別稽核：答案自稱「最新一季」，卻引用了較舊的期別

### 一次誤診值得先寫下來

`web-03` 的第一份根因分析是這樣推的，每一步都對，結論卻錯：

1. Grader 對「Microsoft 最新一季的 Azure 營收成長率」判 `realtime_need="none"` — **照 prompt 做對的**，`none` 的定義逐字寫著「某季營收」。
2. `REALTIME_STALE_DAYS["none"] is None` → `_stale_for_realtime()` 第一行 `if limit is None: return None` → **`_kb_ceiling_date()`、`kb_unfixable`、天花板比對整段被短路，連 `as_of` 都沒讀到**。
3. 於是「KB 補不了就去 web」那條路被一個三分類的 LLM 欄位單點守住。
4. → 結論：該把「相對期間指稱」拆成獨立的軸，不被 `none` 短路。

第 2、3 步的機制描述**是真的**（見 [`BACKLOG.md`](../BACKLOG.md)）。錯在第 4 步把 `web-03` 當成它的證據。把 coverage 印出來才發現：`MSFT_10K_2026` 的 stamp 是**財年**不是日曆期末，`_source_newest_date()` 刻意把它算成 12/31；而該檔實際涵蓋到 2026-06-30，距 as-of 46 天。**KB 一點都不缺，按那個修法會把一題 KB 明明答得出來的問題送上網。**

> 這是 CLAUDE.md〈web 這條路的診斷成本主要在觀測性〉的又一次印證：**先把要判斷的東西印出來，再猜成因**。這次「印出來」只花了一次 Qdrant scroll。

### 真正的缺陷

MSFT 最新一季是 **Q4 FY2026（4–6 月）**，它**沒有獨立的 10-Q**（被包進 10-K）。系統把「最新一季」解析成「候選裡最新的那份 **10-Q**」＝`202603`，而不是「該公司在證據集合裡最新的**期別**」。更新的 `MSFT_10K_2026 #129`（`Azure ... grew 41%`）就在 commit 集合裡、rerank rank 4，沒被引用，答案也沒說「最新單季 KB 沒有獨立揭露」。

### 三個設計決定

**① 判準看答案，不看問題。** 判「使用者是不是在問最新一期」要嘛加詞表（換措辭就漏），要嘛在 Grader prompt 加欄位——但那會動到 snapshot 的 Checker prompt，破壞 `verify_web_gate_isolation.py` 常駐把關的「eval 基準逐字不變」。轉個角度：**答案寫出「最新一季」時，它就是在做一個宣稱**，而「你引的是不是證據裡最新的一期」是純比對。這是驗證答案自己的宣稱，不是猜使用者意圖 → 誤報風險低（問 FY2025 時答案不會寫「最新一季」），漏判只是維持現狀。

**② 排序不能用 `_source_newest_date()`。** 它把 `TICKER_10K_YYYY` 一律算成該年 12/31（刻意高估新鮮度，見其 docstring），而各家財年結束月份不同：
...
期別欄位不在 `rq.retrieve()` 回傳的 chunk dict 裡（只有 `source/chunk_index/ticker/chunk_type`），靠**擴充 `_scan_kb_coverage` 的同一次 scroll** 建 `source → (fiscal_year, fiscal_period)` 表——比改全專案共用的 retrieve 投影安全。

**③ 每家要比兩輪。** 只比「全期別最新」會漏，而且是**修好第一輪之後重生成的答案自己暴露的**：它照指示引了 10-K 並說明「全年 41%、未拆單季」，卻拿 `MSFT_10Q_202512`（FY2026 Q2, 39%）當「最新單季」，而 FY2026 Q3 就在證據裡。第一輪對這一版是全綠的——引到的最新一份確實就是證據裡最新一份。**「最新一季」問的是最新的「季」，10-K 過關不代表季別選對。**

### 宣稱詞辨識：窮舉清單在同一次迭代內就漏了兩個

第一版寫成 `最新一季|最新季度|最新財季|…` 的窮舉。修正後重生成的答案寫的是「最新**單季**」與「最新公布的**季報**」——**兩個都不在清單裡，validator 對自己造成的新問題視而不見**。改成「錨詞（最新／最近）＋ 5 字內期別詞，中間不許有數字」；數字那條讓「最新的 2026 年財報」這種**絕對期間**不誤觸。

> 這比 CLAUDE.md 的一般警告更具體：**窮舉措辭必漏，而且往往漏在你自己下一輪產生的措辭上。** 詞表能用的前提是「錨詞 + 結構」而不是「列舉實例」，且失敗方向要釘在漏判側。

### 時效警語同步從「看問法」改成「看結果」

`_build_todo_temporal_scope` 原本用 `bool(_RELATIVE_TIME_RE.search(task)) and rq.looks_like_news_query(task)` 兩個詞表**串聯**決定要不要產生時效缺口。`web-03` 過得了前者（有「最新」）卻過不了後者（不是新聞措辭）→ **一句時效警語都不會印**。

改成 `_news_freshness_gaps(chunks, as_of)`：看**這個子問題實際 commit 了誰的新聞 chunk**。零詞表，而且更準——問法像新聞、答案其實全靠財報時，舊版會印一句無關的新聞 cutoff 警語，新版不會。`_RELATIVE_TIME_RE` 已從碼上刪除。

⚠ **A6 宣稱「兩道詞表閘門全數移除」時只改了 web 觸發那道**，警語這道漏改，而 `CLAUDE.md` 已經寫成全移除——**碼與文件漂移了兩天**。這類「同一個名字出現在兩處、只改了一處」的漏改，`grep` 一次就找得到，值得在宣稱「全數移除」前跑一次。

### 驗收

[`eval/verify_answer_validators.py`](../eval/verify_answer_validators.py)（零 LLM、零網路，只讀 Qdrant coverage，秒級）五道閘門 **36 項全 PASS**，其中 **10 條是陰性對照**——沒有它們，一個永遠回 `True` 的實作也會全綠。變異注入 3/3：把 `_fiscal_rank` 換回 `_source_newest_date` 排序 → NVDA 那條立刻誤報；停用 validator → `web-03` 那條立刻 FAIL。既有 79 項 web 閘門回歸 PASS。


## A10 2026-08-19 R4：web ↔ 財報衝突要求「並陳」而不是「裁決」

**缺口**：`find_authority_conflicts`（R3）只看 `chunks` 的 `src_types`，而 live web 的內容走
`web_extra` **字串**、不是 chunk → `_ground_source_type` 看不到它 → 只出現在 web 的數字拿到空的
`src_types` → R3／R4 一律跳過。KB 拔除新聞後 web 成了**唯一的即時來源**，這條風險反而變重要。

**為什麼不擴充 R3**（這是整條的核心判斷）：R3 的動作是「**判新聞那個為錯**」。那對 KB 新聞成立
——它在構造上就是舊的，而財報對同一期別有權威性。**web 不是這樣**：它可以合法地比任何 filing
都新（盤中報價、8-K 事件、財報已發布但 10-Q 未申報）。宣告「財報永遠對」會讓系統**系統性地
報舊數字**，正是 CLAUDE.md 那句「把三週前的數字講成『今天股價』」的鏡像。
⚠ [`agentic_rag_v2.py`](../agentic_rag_v2.py) `_consistency_check_and_fix` 的 docstring
**早就寫下「把 web 併進去會改變 R3 的語意」**——那句話是對的，只是結論該是「新增一條動作不同的
規則」而不是「所以不做」。

**R4 的動作**：同一格出現互斥數值、一邊只在 web、另一邊在財報 → 要求答案**兩個都講、各自標
出處與時點**。兩邊都已標出處 ＝ 模型已經並陳了 → 沉默。

**為什麼「並陳」在誤報下是安全的**（選這個動作而不是裁決的主要理由）：
最大的誤報來源是「同一指標、不同期間」——`period` 刻意不入 bucket key（沿用 R3 的理由：
web／新聞幾乎不標精確期間，納入會讓兩邊永遠落到不同桶、規則永不觸發）。而誤報時要求模型做的
事是「把兩個值連同各自的時點講清楚」，**那對不同期間的兩個值本來就是正確行為**。
裁決型規則沒有這個性質：判錯一次就是把對的數字刪掉。

**能力上界**（寫清楚免得被當成保證）：claims 是從**答案**抽的，所以 R4 只看得到「答案自己講了
兩個互斥的值」。答案只講其中一個、而另一個來源有不同值的情況**看不到**——那要從來源側反向抽取
指標，不是確定性能做的事。

**順帶修掉的一個洞**：`_ground_source_type` 原本開頭是 `if not chunks: return`，於是
「候選池空、只有 web」——**那正是 live 路徑的常見形狀**——整個跳過 grounding。閘門⑧ 抓到的。

**驗收**：[`eval/verify_answer_validators.py`](../eval/verify_answer_validators.py) 閘門⑧（15 項，
零 LLM）。⚠ **重點在誤報對照那組**：R4 的動作在誤報下也正確，所以真正會出事的方向不是漏抓，
是**話太多**（每個財報題底下都掛一段要求並陳）。雙向變異測試：拿掉機制 → 2 條 FAIL；
拿掉「已標出處就沉默」的條件 → 誤報對照 FAIL。
