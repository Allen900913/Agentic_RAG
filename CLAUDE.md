...
| [`eval/probe_gold_funnel.py`](eval/probe_gold_funnel.py) | **撈到了 → 有沒有用**（零 LLM、零檢索，只讀結果檔＋Qdrant）。三段：`gold ∈ retrieved_union`（撈到過）→ `gold ∈ sources`（撐過 Grader 圈選＋`COMMIT_TOP_K`）→ `gold ∈ cited`（真的引用）。⚠ **第一段要 `retrieved_union`（2026-09-09 起才有）**；舊結果檔只跑得動兩段版，而兩段版的 ②（撈到了被丟掉）與 ③（真的沒撈到）**一起落進 ③** → 報表分群印、**兩群不可合併**。⚠ 引用是 LLM 自己宣稱的 ⇒ 每一段都只會**高估**，**低分是硬證據、高分不是健康證明**。⚠ 引用解析一律用生產的 `_extract_citations`，不自己寫 regex |
