...
| [`eval/audit_gold_numbers.py`](eval/audit_gold_numbers.py) | **gold 自洽性稽核（零 LLM、零網路）＝跑任何評測前的前置閘門**。三項：① 雙寫自洽（`N 億美元（$M billion）` → N/10 = M）② 成長率自洽（同句 `A→B` 與 `X%` 要能驗算）③ 小來源定位（gold_files 總量 < 40 KB 才做）。⚠ **不查 filing 類的數字**——第一版就是「拿回申報檔定位」，實測四種注入錯誤全漏抓（10-Q 有 17,268 個數字，1% 容差下隨機值 94/100 都配得到），與 `check_number_defects` 前三版同一種失效。`--self-test` 雙向驗判別力（真錯要抓到＋正確寫法不能誤報）。2026-08-11 基準：6 題 14 筆、**零筆真錯**（13 筆是 gold 精度高於語料、1 筆正當改寫） | 生產 `.venv` |
