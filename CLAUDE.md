...
| [`eval/verify_period_intent_routing.py`](eval/verify_period_intent_routing.py) | **驗收期間意圖路由的 Python 半邊（零 LLM，只讀 payload，秒級）**。五道閘門 39 項：①`_fiscal_sort_key` 真值表（含 NVDA 財年反轉）②**ladder 排序 vs 期碼字串比大小**③`ladder_pick`（挑不到必須回 None，**不可退而求其次**）④`period_ref` → filter 對應（只有 `latest` 該產生 filter）⑤`range` → 跳過 collapse 的接線。⚠ 閘門② **在單年 collection 上幾乎沒有判別力**，腳本會自己印出「判反幾對」並在 0 對時明說**那是測資不足不是系統健康** | 生產 `.venv` |
