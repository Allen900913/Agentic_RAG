...
### E1／E2：sweep 的收益在 agentic 上是 0、內容天花板旗標的證據翻面、EPS 族是 gold 太窄

**E1（零成本，讀既有檔）**：被 RRF sweep 救回的三題，在 R3（RRF=20）的 agentic 管線上
**本來就沒有損失**——`lex-17` 靠 ratio 保底、`mix-14` 靠跨子問題聯集已經答對，
`sem-03` 的 gold **已經在 `retrieved_union` 裡**、卡的是 Grader 圈選不是名額。
⇒ 那 +0.049 `gold@5` 在 agentic 上找不到通往答案的路徑。**常數仍然保留 30**：
收益落在單管線（web UI，一次檢索、top-5 即全部），而 **+46% 的成本兩條路都付**
⇒ 記成〈已知帶著上線的取捨〉，不是一個乾淨的勝利。

**E2（~30 分，零 LLM 判定）**——本來要問「HyDE 該不該開」，結果查出兩件更重要的事：

**① EPS 族不是召回失敗，是 gold 定義太窄。** `lex-03`／`lex-07`／`lex-14` 三題
**全部答對**（4.90／7.46／23.49）且引用 10-K 的損益表，而 `chunk_gold.json` 把 gold
釘在 `*_IncomeStatement_*.txt#1`。⇒ 已把三題的 `relevant` 加上對應的 10-K
（`eval_set.json` 與 `chunk_gold.json` **兩邊一起改**——selftest ③ 要求逐字同源，
只改一邊會當場 FAIL，那道斷言這次真的擋下來了）。gold 83 → 95 顆，
`--check` 0 漂移、`--selftest` 全過、`audit_gold_numbers` 與 HEAD 逐字相同。
⚠ **`gold@5 = 0.683` 這個頭條數字因此低估了系統**，「32% 有檢索層缺陷」裡至少 3 題是量尺造成的。

**② 內容天花板旗標的證據翻面了，決定不做。** 原本的依據是「陰性對照的內容誤殺 0/15」。
逐筆讀 R3 的 `forced_pass_detail` 並**直接讀 Qdrant 驗證**（不採信答案自己的引用宣稱）：

| 子問題 | Grader 說缺什麼 | 實情 |
|---|---|---|
| `lex-04` | 「CUDA 開發平台的推出年份」 | `NVDA_10K_2026.html#20` 逐字寫著「With our introduction of CUDA in 2006」 |
| `mh-01` | 「Alphabet TTM revenue is not present」 | `GOOGL_Fundamentals#0` 寫著 `Revenue (TTM) : $422.50B` |

⇒ **8 個 forced_pass 子問題有 2 個是 Grader 判錯，事實就在 KB 裡 ⇒ 誤殺率至少 2/8 ＝ 25%。**
⚠ **那個 0/15 為什麼看不到**：陰性對照抽樣自**生產判夠**的子問題，而危險的形狀正好是
「生產判**不**夠、但事實在 KB 裡」——**取樣框架結構上排除了它**。這是「量尺自備輸入」
的另一種形狀：**陰性對照的來源決定了它能看到什麼**。
⚠ 代價不對稱且方向是反的：旗標若在 `lex-04` 上亮，會把一個逐字有據的正確答案**變成拒答**。
⇒ 進〈已接受的極限〉，並寫下復活條件（要有能分辨「Grader 判錯」與「KB 真的沒有」的尺）。
⚠ 我原本在 CLAUDE.md 舉的例子就是「10-K 沒寫 CUDA 推出年份」——**那句話本身是錯的**，已更正。

**③ 順帶查到一個潛在缺陷**（無害但記下來）：`*_IncomeStatement_*.txt` 的
`--- Fiscal Year: YYYY ---` **只出現在 `#0`**，`#1..#N` 沒有年份；`META_IncomeStatement`
的 `#1`（FY2025 `$23.49`）與 `#6`（FY2024 `$23.86`）**都沒有年份標記**，
而 `#4` 的財年標頭出現在塊**中間** ⇒ 單一 chunk 橫跨兩個財年。修法要重跑 ingest，
**等下次有正當理由重建時一起做**，別為它寫檢索層特例。

### E6：gold 修完之後重量——「檢索層有 32% 缺陷」剩下 12.2%，而名額問題整個消失

`experiments/cgr_rrf30_goldfix3_20260911.json`（41 題 × 3 輪，翻面 0）／
`experiments/rla_r2_20260911.json`（41 題 × 2 輪，翻面 0）。

| | 09-08 頭條 | 修 EPS gold | **修 literal 邊界** |
|---|---|---|---|
| `gold@5` | 0.683 | 0.805 | **0.878** |
| `gold@20` | 0.780 | 0.902 | **0.976** |
| 召回全滅 | 9 | 4 | **1**（`sem-05`） |
| 排序損失 | 4 | 4 | **4**（三輪都同樣四題） |
| 有缺陷 | 13/41＝32% | 8/41 | **5/41＝12.2%** |

**9 → 1 的拆帳**：**2 題真的修好**（`lex-17`／`sem-03` 靠 RRF 20→30 進池）、
**6 題是量尺錯**（EPS 族 gold 太窄 3 題；裸年份 literal ＋ 邊界 bug 3 題）、1 題仍在。

**兩個寫在 CLAUDE.md 裡的判讀因此作廢**：
1. ~~「lexical 的 `@5` 與 `@20` 完全相同（30/30）⇒ 那一類的病 100% 在召回」~~
   ——lexical 現在是 **1.000／1.000**。當初的「完全相同」是因為那 5 題的 gold 都指錯地方，
   `@5` 與 `@20` **一起全滅**，看起來才「完全相同」。**那是一個由缺陷製造出來的漂亮規律。**
2. ~~「9 題召回＋4 題排序」~~ → **1 召回＋4 排序。問題的主體從召回翻成排序。**

**層級歸因四個名額格全部歸零**：① collapse／filter 刪掉 gold 0／② RRF 名額 0／
②深 0／③ prefetch 名額 0／④ 真的撈不到 **1**（`sem-05`）／沒問題 40。
⇒ ~~「五題是名額問題，`RRF_TOP_N_PRIMARY` 與 `FETCH_N` 的 sweep 因此有據可循」~~
**那個結論整個來自 gold 缺陷**，而它正是 09-10 那整輪 sweep 的動機。
⚠ 09-11 新加的「切開 RRF 與 FETCH_N」那條臂**一次都沒被走到**（②／②深／③ 全 0）
⇒ 它的判別力目前**只存在於 selftest**，不要說「實測分得開」。

⚠ **這連帶讓 `RRF_TOP_N_PRIMARY` 20 → 30 的理由需要重算**：當初的 +0.049 `gold@5`
是在**有缺陷的 gold** 上量的，而成本（單次檢索 28.8s → 41.9s，+46%）是真的、
而且**單管線那條使用者實際在用的路上每一次查詢都在付**。**E7 已在修好的 gold 上重測**（`experiments/cgr_rrf20_goldfix3_20260911.json`，41 題 × 3 輪）：

| RRF | `gold@5` | `gold@20` | 召回全滅 | 排序損失 |
|---|---|---|---|---|
| 20 | 0.829 | 0.927 | `sem-03` `sem-05` `lex-17` | `sem-04` `mix-08` `mix-14` `col-03` |
| **30**（現行） | **0.878** | **0.976** | `sem-05` | `sem-03` `sem-04` `mix-08` `col-03` |

**差異恰好三題，而且三輪一致（0/3 → 3/3，沒有任何抖動）**：`lex-17` 進池、
`mix-14` 進 `@5`、`sem-03` 進池。⇒ **收益是真的、可複現，`+0.049` 與在有缺陷的 gold 上
量到的數字完全相同** ⇒ 先前的疑慮排除，**維持 30**。
⚠ 但 E1 的結論不變：那三題在 agentic 上本來就沒有損失（`lex-17` 走 ratio 保底、
`mix-14` 靠跨子問題聯集、`sem-03` 在 RRF=30 下仍卡在 Grader 圈選＝漏斗的 ② 格）
⇒ 這仍然是〈已知帶著上線的取捨〉，不是一個乾淨的勝利。

### 量尺為什麼一直錯：歸類成四個根因，三個機械可判定的做成一道閘門

問題是「為甚麼量尺一直錯誤」。把失效史全部攤開（6 次耦合 ＋ 6 次 `check_number_defects`
判法被推翻 ＋ 5 批 false-negative gold ＋ 今天新增的五個）之後，**只剩四個根因**：

| 家族 | 形狀 | 今天怎麼修 |
|---|---|---|
| **A 抄寫** | 量尺把生產組態抄成字面常數 ⇒ 改生產的那一刻脫鉤，而量尺照跑照印 | 閘門 **H3**：常數值被凍結，一變就 FAIL 並列出所有抄寫點要求複核 |
| **B patch 點不符** | eval 攔 `ar.<name>`，呼叫端是裸名 ⇒ stub 蓋不到而閘門全綠 | 閘門 **H2**：patch 名單從 eval **反推**，逐一驗套件無裸名引用 |
| **C 定位規則自己有 bug** | ← **今天的主角，見下** | 修 `literal_matcher` 的邊界 ＋ 8 條斷言 |
| **D 量尺壞掉與系統壞掉外觀相同** | 報表 crash 被讀成 FAIL、N/A 被併進 PASS、coverage 太窄卻回報「0 筆」 | 閘門 **H1**：會印非 ASCII 的腳本一律要轉 stdout **與** stderr |

**C 的根因：`chunk_gold.literal_matcher` 的數字邊界把「數字延續」與「標點」混為一談。**
邊界是 `(?<![0-9,.])…(?![0-9,.])`（本意是不讓 `1,353` 吃到 `21,353`），於是
**任何後面緊跟逗號或句點的數字一律匹配不到**——而英文散文裡數字後面最常見的正是這兩個。
實測 `lex-04` 的 gold 因此指到兩顆**附件索引**（`3/7/2006`，後接空白所以匹配得到），
而真正的答案 `With our introduction of CUDA in 2006, we opened …` 匹配不到。
改成只排除真正的延續（`(?<!\d)(?<!\d[.,])…(?!\d)(?![.,]\d)`）：
**gold 95 → 102 顆、3 題受影響，而那 3 題（`sem-01`／`sem-05`／`col-01`）正是漏斗
③「檢索根本沒撈到」那一格剩下的四題裡的三題。**

**兩次 gold 修正的累積效果（R4、`experiments/gold_funnel_r4_20260911.json`）**：
漏斗 ③「檢索根本沒撈到」**5 題 → 4 題 → 2 題**（只剩 `sem-04`／`sem-05`），
「撈到了也被引用」30 → 31 → **33 題（80%）**。
⇒ **原本讀成「檢索層缺陷」的那一格，大部分是量尺。** ② 那格（撈到了被圈選丟掉）不動，
仍是 5 題 ＝ **現在檢索層與圈選層的比例是 2 : 5**，而先前的讀法是 5 : 5。
⚠ 自測表裡本來就有 `("2006", "CUDA in 2006", True)` ——**它把生產文字的逗號拿掉了**，
於是壞掉的那條路從來沒被測到（同一形狀本 repo 已踩六次）。新增 8 條，4/4 變異全抓到；
其中「只修後方」那個變異一開始**沒被抓到** ⇒ 補了兩條真實形狀（SEC 的點引線 `......1,353`
與逗號緊貼 `EPS,4.90`）才抓得到前方邊界。
⚠ `sem-01`／`col-01` 另外改成片語 literal：邊界修好之後裸年份會**連附件索引與 CFO 經歷
一起收進來**，而 **gold 有雜訊會讓召回率虛高**（撈到一顆附件索引就算命中）。

**新閘門 `eval/verify_eval_harness.py`**（零 LLM／零網路／零 Qdrant，19 項自測、4/4 變異）
掃完當場抓到三件事：

1. **H1：51 支裡 27 支**會在 cp950 主控台印到一半 crash（其中兩支今天真的踩了：
   `check_comparison_claims` 5/5 全 PASS 卻 crash 收場、`check_number_defects` 摘要整段不見）。
   補完之後又抓到 **7 支只轉了 stdout**——`verify_answer_validators` 的註解甚至寫著
   「cp950 會在 gate8 當場炸」，然後只修了 stdout。**traceback 走 stderr。**
2. **H2：`_classify_staleness` 在 `graph.py:417` 被裸名呼叫**，而 eval 攔 `ar.` 上的綁定
   ——那正是今天害我 debug 一輪的 ⑰f。而當時的修法是**把注入點搬到定義處** ＝ 遷就缺陷。
   ⇒ 呼叫端改成 `_pkg.`、注入點搬回 `ar.`。**接著連鎖爆出兩層**：⑬a 少列這個名字
   （因為先前攔在 `_g` 上，它的 AST 反推看不到）、⑬b 抓到 `graph.py:47` 的
   `from .validators import _classify_staleness` **壓了一份綁定**＝真正的根，已移除。
   ⇒ **教訓：量尺為了「能攔到」而搬注入點之前，先問「是不是呼叫端該改」。**
3. **H3：`probe_kb_content_ceiling` 的 docstring 寫著「`RRF_TOP_N_PRIMARY` 20→50」**，
   而生產 09-10 已是 30 ＝ 過期的抄寫，連帶讓那份證據檔的「3 倍寬」逐字不成立（實際 1.67 倍）。

⚠ **H3 刻意不猜**：「這個字面 `20` 是不是抄來的」是語意判斷
（`MAX_REWRITES=2` 與任何字面 `2` 都撞，17 個候選裡多數是誤報）。它改成兩條精確斷言
——每個候選要登錄並寫下理由、常數一變就要求重新複核。**那正是 09-10 缺的那一步。**
⚠ **C 家族不在這道閘門裡**，也不該在：literal 的語意對不對只有雙向自測管得到。
⚠ **E 家族（取樣框架排除了目標，見 `probe_kb_content_ceiling` 的 0/15）機械上無解**，
   它是設計複核項，寫在 `docs/EVAL.md`。
...
| 組態 | gold@5 | gold@20 | 召回失敗 | 排序損失 | 有缺陷 |
|---|---|---|---|---|---|
| RRF=20 ＋ 舊 gold（09-08 頭條） | 0.683 | 0.780 | 9 | 4 | 13/41＝**32%** |
| RRF=30 ＋ 舊 gold | 0.732 | 0.829 | 7 | 4 | 11/41＝27% |
| **RRF=30 ＋ 修過的 gold（現況）** | **0.805** | **0.902** | **4** | 4 | **8/41＝19.5%** |

⚠ **拆帳一定要拆**：召回失敗 9 → 4 裡，**2 題是真的修好**（RRF=30 讓 `lex-17`／`mix-14`
進 @5）、**3 題是量尺本來就錯**（EPS 族 gold 太窄，見上一條 E2①）。
而 E1 已經證明那 2 題在 agentic 上本來就答對了 ⇒ **這個數字變好，與答案變好之間
仍然沒有建立起任何連結。** 剩下的 4 題召回失敗是 `sem-01`／`sem-05`／`lex-04`／`col-01`
（`lex-04` 正是 E2② 那題——gold 沒進池，而最終答案靠別的 chunk 答對了）。

### E3b：層級歸因重跑——RRF 名額那一格歸零，但 ③ 格的標籤在說一件它分不出來的事

`experiments/rla_20260911.json`（41 題 × 2 輪，RRF=30 ＋ 修過的 gold，**跨輪翻面 0**）：

| 格 | 09-09（RRF=20、舊 gold） | 09-11 |
|---|---|---|
| ① collapse／filter 刪掉 gold | 0 | **0** |
| ② RRF 名額（RRF=100 才進得來） | 2 | **0** |
| ③ prefetch 名額 | 3 | 3（`sem-01`／`lex-04`／`col-01`） |
| ④ 真的撈不到 | 3 | **1**（`sem-05`） |
| 跨輪翻面 | 1 | **0** |

**② 歸零是 `RRF_TOP_N_PRIMARY` 20 → 30 的直接後果**，也是這次唯一乾淨的讀數：
名額那條懷疑到此結案。① 仍是 0 ⇒「collapse／hard filter 刪掉 gold」兩次量測都不存在。

⚠ **但 ③ 那 3 題不可以讀成「該調 `FETCH_N`」，而舊標籤正是那樣寫的。**
兩條寬臂是 `RRF=100／FETCH_N 生產值` 與 `RRF=200 ＋ FETCH_N=300`——**後者兩個旋鈕
一起放寬** ⇒ ③ 的真正語意是「**兩個一起放寬才進得來**」，分不出是哪一個。
而 `docs/EVAL.md` 已經記著「`FETCH_N` 單獨調對 `retrieve()` 的回傳零效果」
⇒ 照舊標籤讀，會**第二次得到同一個已經被否決的行動建議**。

修法是補第三條臂（`RRF=200`、**`FETCH_N` 維持生產值**）把兩者切開：進得來 ＝ ②深
（仍是 RRF 名額），仍進不來才是 ③。`classify` 四格 → 五格，selftest 13 → 18，
新增的 ⑧b／⑧c／⑪／⑪b 全是誤報對照，**4/4 變異全抓到**（拿掉那一刀 ＝ 退回舊行為，
⑧b 當場響）。⚠ 同一次抓到 `_meta` 的 `"arms": {"prod": [20, 60]}` **也是寫死的**
——那比報表標籤更糟：**假話會被永久寫進證據檔**。

⚠ **`experiments/rla_20260911.json` 就是舊設計跑出來的**，讀它要記得兩件事：
`_meta.arms.prod` 寫著 `[20, 60]` 而實跑是 `[30, 60]`（生產臂已修、`_meta` 當時還沒），
且 ③ 那 3 題**尚未歸因**。**重跑排在 E4 之後**——生產臂要跑 reranker，
現在跟全量搶 CPU 會拖慢 E4 且量不到更多東西。

### `probe_recall_layer_attribution` 的生產臂把常數寫死（量尺與被測物耦合，第三次）

該支的生產臂是 `_one_arm(..., 20, 60)` ——**字面常數**。`RRF_TOP_N_PRIMARY` 09-10
改成 30 之後，**它從那一刻起量的是沒有人在用的組態，而外觀完全正常**（照跑、照印數字）。
報表標籤同病（「進過 RRF-20」）。改成 `rq.RRF_TOP_N_PRIMARY, rq.FETCH_N`，
標籤改由 `_labels()` 從常數算出；寬臂的 100／200／300 **刻意保持絕對值**
（那是「名額夠不夠」的上界探測，不是生產組態的倍數），但提到模組層常數不再重複寫。
selftest 8 → 13，新增的 ⑨b／⑨c／⑨d 三條全是誤報對照（寫死的版本／只 import 一半／
整段不存在時不可真空成立）。

⚠ **同一個形狀本 repo 已踩三次**：閘門① 的 `_gate()` 是抄寫、`probe_chunk_gold_recall`
把「上限 20」寫死在輸出字串、這次是生產臂本身。
⇒ 通則寫進碼裡：**量尺凡是要重現生產行為的地方，一律 import 不抄寫。**
...
`cgr_rrf50_20260910.json`（41 題 × 3 輪 × 三臂，掛 replay cache 釘死英譯）。
**基準臂逐字重現 09-08 的 `gold@5 0.683`／`gold@20 0.780`** ⇒ 差異可歸給該常數。

| RRF | 秒/次 | 相對 20 | `gold@5` | `gold@20` |
|---|---|---|---|---|
| 20（現行） | 28.8 | — | 0.683 | 0.780 |
| **30** | 41.9 | **+46%** | **0.732** | **0.829** |
| 50 | 67.8 | +135% | 0.732 | 0.829 |

**改善 3 題、退步 0 題、跨輪不穩定 0 題**（`lex-17`／`mix-14` 進 @5，`sem-03` 進 @20）。
`mix-14` 是意外收穫——它本來是**排序**問題，放寬名額後 rerank 從更大的候選集挑中了它。

⚠ **50 相對 30 一題都沒多買**（逐題 k/n 逐字相同）→ 進〈已試無效總表〉。
成因：`RERANK_INPUT_N=50` 是重排輸入上限，RRF 回 50 就是 50 個全進重排，
而**第 31~50 名沒有一個含 gold**。

⚠ **`FETCH_N` 那一臂是 null result**（同日稍早）：60→180→300 對 `rq.retrieve()` 的回傳
零效果，因為 prefetch 之後還串著 `RRF_TOP_N_PRIMARY`。⇒ 這也修正了 T1 結論 2 的後半句
（「`FETCH_N` 比 `RRF_TOP_N_PRIMARY` 更值得動」）——T1 的歸因沒錯，錯的是我把它讀成
一個可以單獨轉的旋鈕。

⚠ **常數沒有動，這是刻意的**。收益是「41 題裡 2 題進 @5」且量的是**檢索不是答案**
（`chunk_gold` 是聯集 ⇒ 上界；而同日的三段漏斗顯示 `forced_pass` 多半不是檢索問題）；
代價是使用者實際等的那條路 **28.8s → 41.9s**，agentic 還要乘上子問題數。
**這是產品取捨不是分數取捨。** 另外 `rag_query.py` 的註解寫著「sweep 顯示 20 > 40」
（舊 collection，已作廢）——要換常數就得在同一次把註解與證據檔一起改，
**不可以偷偷改掉註解再換值**。

⚠ 量尺：`probe_chunk_gold_recall` 加了 `--override`（selftest 8 → 12），
刻意不另寫第二支 sweep 腳本、也刻意不給生產加 env 入口。
