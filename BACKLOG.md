...
## 已知缺陷：mi-05（**仍然成立**；2026-08-19 在去掉新聞子句的 `lex-17` 上重現）

> **2026-08-19 更新（先讀這裡）**：把原題**逐字沿用前半、只拿掉新聞子句**變成 `lex-17` 端到端重跑兩次——
> **r1 PASS（`#0` 進池、答 18.30% 標明 TTM）／r2 FAIL（撈到 `#1`、改答 10-K 財年 18%、增加 501 億美元）**。
> → **拿掉新聞子句沒有修好它。** 缺陷在單意圖題型上一樣會發生。
>
> ⚠ **我一度只憑 r1 就宣告「根因是新聞污染、rerank 長度偏好假設不成立」，並改掉了本節的標題與結論**，r2 跑完立刻推翻。這與本節下方 2026-08-12 那條自我更正說的是同一件事——**它不是穩定行為，部分取決於 Plan 產生的子問題**——我讀到了那句話卻仍然用 1 個 run 下結論。
> 所以舊記載真正錯的只有「4/4 穩定」那個**強度**（已於 2026-08-12 自我更正成 8/10）；「懷疑 #1 長三倍造成 rerank 偏好」**仍未驗證，仍然不是結論**。
>
> 真的被拔除新聞修好的是 **`mix-07`**（2/2 從拒答翻成答出 $29.5B），別把兩題的結論互相套用。
>
> 現況：`lex-17` 在 `eval/number_claims.json` 掛兩條——`require_chunk`（`known_defect`，缺陷記帳）＋ `anchored_pct`（`regression_guard`，口徑護欄，**刻意不填 forbid**，理由見該條 `metric_design`）。
