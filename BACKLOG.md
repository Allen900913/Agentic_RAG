...
- **【`eval/judge_regression.py` 跑不起來】**（2026-08-27 發現，**既有壞掉、不是當天造成的**）。`TypeError: evaluate_correctness_with_feedback() got an unexpected keyword argument 'hall_result'`。已用 `git stash` 退掉當天改動跑同一支確認錯誤一模一樣。**等於 judge 現在沒有回歸保護**——而 judge 的行為正是「量尺對不對」的最後一道。
- **【「證據尾巴」有四份各自的定義】**（2026-08-27）。`agentic_rag_v2` 的 producer、`eval/check_number_defects.FOOTER`、`eval/check_rounding_fidelity._TAIL_RE`、`eval/eval_ragas_vs_rubric` 的 footer strip。正式定義已收攏成 `rq.strip_evidence_tail()`，**新消費端一律用它**；既有那四份沒有動——動 RAGAS 那份會移動分數，要單獨做並重新取基準。
