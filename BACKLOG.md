...
- **「最新期間」路由的覆蓋面，會隨 KB 累積年份惡化**（2026-08-14 提出）。病灶本身**已實證且已部分修好**：cross-encoder 分不出同主題 chunk 是哪一季（mix-06 probe：舊季 0.70~0.73 與最新季幾乎並列，最新季 gold 被壓到 rank 7/13/17、掉出 top-5 → context_recall=0），[`rag_query.py`](rag_query.py) `_LATEST_QUARTER_TIME_RE` 那條 deterministic 路由就是治本修法——掃 collection 解出最新 10-Q 期碼、注入 `report_period_code` 硬 filter。**但它有三個破口，且排序層完全沒有時間先驗**（[`rag_query.py:1429`](rag_query.py#L1429) 純 `raw_rerank_score` 排序）：
  ...
  **剩下的卡點只剩一個，而且是缺量尺不是缺修法**：現行基礎上，多年＋A＋B 相對單年生產的代價是 **45 題掉 2 題、錯期率 0.000 → 0.044**（兩題都在「問題沒提期間」那一類）。但**沒有任何量尺在量多年語料買到了什麼**——`eval_set.json` 沒有一題的 gold 落在舊年度 filing，趨勢題那 8 題量的是期別覆蓋不是答案正確性。**只有代價沒有效益，不足以做升生產的決定。**
  **要補的量尺（最小版）**：一組 gold **只存在於舊年度 filing** 的歷史題（例如「Apple FY2024 的總營收是多少」）。它在單年 KB 上結構性地答不出來 → 收益是**類別型的不是邊際型的**，n 不必大。⚠ **不要併進 `eval_set.json`**（分母又變一次，跨日分數再也比不了）。
  升生產時要一起做的還有：翻 `RQ_PERIOD_INTENT_LLM`（**別分兩次做**，見上）、釘死 `sem-03`／`sem-04`／`col-02`／`col-03` 的萬用字元 gold、修 `MSFT_10K_2024.html#158` 的幅度接地漏抓。
