...
- **`MSFT_10K_2024.html#158` 幅度接地在 chunk 層漏一筆**（2026-08-19，多年語料壓測時 [`verify_chunk_grounding.py`](eval/verify_chunk_grounding.py) 抓到）。
  ...
  **復活條件**：下次有正當理由重跑 ingest 時（不要為了這 0.1% 專門跑一次三小時的重建），在切塊器裡對這一份 filing 印出 segment 邊界，三選一。
  ⚠ **2026-08-27 升生產時重新確認：這條的復活條件沒有被觸發**——升生產只是換 `COLLECTION_NAME`，collection 早就建好了，**不需要重跑 ingest**。所以它是**帶著上線**的（1/1009＝0.1%）。同一天 BACKLOG 另一處曾把它寫成「升生產時要一起做的」，那是寫錯了，已更正。
...
- **「最新期間」路由的覆蓋面，會隨 KB 累積年份惡化**（2026-08-14 提出）。病灶本身**已實證且已部分修好**：cross-encoder 分不出同主題 chunk 是哪一季（mix-06 probe：舊季 0.70~0.73 與最新季幾乎並列，最新季 gold 被壓到 rank 7/13/17、掉出 top-5 → context_recall=0），[`rag_query.py`](rag_query.py) `_LATEST_QUARTER_TIME_RE` 那條 deterministic 路由就是治本修法——掃 collection 解出最新 10-Q 期碼、注入 `report_period_code` 硬 filter。**但它有三個破口，且排序層完全沒有時間先驗**（[`rag_query.py:1429`](rag_query.py#L1429) 純 `raw_rerank_score` 排序）：
  ...
  **~~升生產時要一起做的~~ → 2026-08-27 升生產完成**：`COLLECTION_NAME` 換成 `us_stock_rag_edgar_multiyear`、`RQ_PERIOD_INTENT_LLM` 預設翻開（同一次，見上）、四題萬用字元 gold 釘死。⚠ **`MSFT_10K_2024.html#158` 的幅度接地漏抓沒修、帶著上線**——原本這行把它列成前置條件是**寫錯了**：修它要重跑 ingest，而升生產不需要，它自己的復活條件（見上）並沒有被觸發。規模 1/1009＝0.1%；**下面兩個由收益探針抓到的缺陷都不是多年語料造成的**——「絕對年份 filter」那條更是**現生產就在發生**（已於同日修掉）→ 它們是**獨立的既有缺陷，不是升生產的前置條件**。
- ~~**【多年語料 ②】生成層的收益完全沒量**~~ → **2026-08-27 量完**，完整分析見 [`docs/EVAL.md`](docs/EVAL.md)〈多年語料在生成端買到了什麼〉。量尺是新增的 [`eval/check_historical_generation.py`](eval/check_historical_generation.py)（零 LLM、三態、selftest 12 條）。
  · **擔心的那件事沒有發生**：`拿別年份硬答且沒交代` 在**兩臂都是 0**。檢索層那 33 席「同節別期」沒有兌現成生成層的實害——單年臂 **18/18 誠實承認**（明說缺哪一年、還交代有哪幾年）。
  · **生成端的收益**：答對 **0/18 → 16/18**；當期陰性對照 4/4 不動。
  · ⚠ 這推翻了收益探針輸出裡「那是有引用、看起來很有根據的錯答」那句話——**那是推論不是量測**，已就地更正。
  · ⚠ 量尺被自己的資料推翻三次（誤用 `looks_like_refusal`、詞表漏英文與「未披露／無法給出」、`bh-19` 的 literal 是英文片語），每一次都是**答對了卻被判成危險態**。
- **【收益題庫的前提檢查擋不住「重述」】**（2026-08-27）。`bh-07`／`bh-16` 已標 `void`：`MSFT_10K_2026` 的補充未審計表帶著**重述後**的 `219,790`／`87,464`，單年 KB 其實答得出來，而前提檢查比對的是原始值 `211,915`／`105,362`。**值不存在 ≠ 事實不存在。**
  ⚠ **抓到它們的不是前提檢查，是生成端那一輪**（單年臂把它們答出來了）→ **新增收益題之後兩支都要跑**，值比對一個人擋不住。檢索層數字已同步更正（兌現率 0.900 → **0.944**）。
  **還沒做**：沒有任何機制能一般性地偵測重述（同一事實在新年報是另一個值）。這與既有那條「`filing_date` 沒進 payload → 沒有 transaction time」是同一個根（雙時間軸）。
- **【agentic 沒有揭露通道】**（2026-08-27）。`agentic_rag_v2._retrieve_chunks()` 寫的是 `chunks, _note = rq.retrieve(...)`——**Tier 2 的期間缺口揭露語被丟掉**。單管線（`rag_query.py` CLI／`api_server.py`）兩邊都有（SSE 顯示 ＋ 注入 generator prompt），**產品線走的是單管線**，所以目前不影響出貨行為。
  但它讓「系統會不會揭露」在**所有 agentic 評測上結構性恆為 0**——量到的 0 是在覆述一行程式碼。修法：把 note 帶進 `_retrieve_chunks` 的回傳、併進該子問題的 summary。⚠ 動它會改變既有 agentic 基準的答案文字，要與一次正當的重跑綁在一起。
- ~~**【`eval/judge_regression.py` 跑不起來】**~~ → **2026-08-27 修好**，見 [`CHANGELOG.md`](CHANGELOG.md) 同日。病灶是 2026-07-23 移除自製 hallucination 指標時 judge 少了 `hall_result`／`relevance_score` 兩個參數，本檔沒跟上。⚠ 真正的教訓不是那個 `TypeError`，是**「要燒 LLM 才跑得動」的東西平常沒人跑，於是壞了一個月沒人知道**——所以修法附了一個零 LLM 的 `--dry-run` 接線檢查（AST 讀出實際送出的 kwargs 比對 judge 簽章、每題 expect 的 checkpoint id 不得越界、`compute_correctness` 三態），那種檢查才會被順手跑進閘門巡檢。
- **【correctness judge 目前沒有活的消費端】**（2026-08-27 修 `judge_regression.py` 時發現）。`eval/eval_set.json` 的 **65 題一題都沒有 `rubric`**（`git log -S` 查過：從 100 題定版那次就沒有），全 repo 只有 `eval/eval_set_agentic.json` 5 題帶 rubric，而讀它的 `eval/eval_agentic.py` 是死碼（見下條）。實測 `experiments/agentic/gj_multiyear_prod_r1.json` 65 題的 `correctness` **全為 null**。也就是說 `evaluate_correctness_with_feedback` 這條線現在只有 `judge_regression.py` 自己的 11 個凍結案例在跑。⚠ **這不表示該刪**——生成品質改由 RAGAS answer_correctness 負責是**刻意的**（見 `eval_generation_llm_judge.py` 檔頭 2026-07-23）。要決定的是：rubric 這條線是**正式退役**（那 judge_regression 也該跟著退役）還是**該把 rubric 補回 eval_set**。在決定之前不要再往 judge prompt 加規則——沒有活的消費端，加了也量不到。
- **【judge 判不了「捏造」，因為它看不到來源】**（2026-08-27）。`_CORRECTNESS_FEEDBACK_SYSTEM` 的 FABRICATION SCOPE 規則寫著「a SPECIFIC number ... **that is not grounded in the sources**」，而 `evaluate_correctness_with_feedback` **沒有任何參數會拿到 chunk**——2026-07-23 移除自製 hallucination 指標之後，連 `hall_result` 那個間接管道也沒了。於是這條規則實際上只能靠**合理性**判（gpt-oss-20b 九次觀測）：`sem03` 的「管理層預期 45% CAGR」**1/9**，而 `col01` 的「市佔率 92%」**7/9**——離譜到光看答案就站不住的還抓得到，讀起來合理的抓不到。⚠ **這個數字我寫錯過一次**：只看某一輪 `--repeat 3` 時 col01 是 1/3，據此寫成「兩個反例方向一致」，下一輪 3/3 就推翻了。`sem03` 已在 `judge_regression.py` 標成 `known_limitation`（失敗記 N/A 不記 FAIL，附復活條件）。⚠ **不建議把 chunk 塞回 judge**：groundedness 這個軸已經由 RAGAS faithfulness 負責，那正是當初移除的理由；真要修，該修的是**題目的 mn checkpoint 措辭**（改成 judge 拿得到的資訊判得出來的形狀），或接受這條軸不由 rubric judge 負責。與上一條綁在一起決定。
- **【`eval/eval_agentic.py` 是死碼】**（2026-08-27）。它 `--module` 預設 `agentic_rag_mix`，而 `agentic_rag_mix`／`agentic_rag_nv` **在 repo 裡都不存在了**（只剩 `agentic_rag_v2.py`）；它呼叫 judge 的地方也帶著同一組已被移除的 `hall_result=`／`relevance_score=`。`run_agentic_on_evalset.py` 的檔頭早就寫明「為什麼不用 eval_agentic.py」。⚠ **沒有跟著修**：修好 kwargs 也還是跑不動（import 的 module 不存在），半修反而讓它看起來像活的。要嘛刪、要嘛在檔頭第一行標明已退役。
- **【「證據尾巴」有四份各自的定義】**（2026-08-27）。`agentic_rag_v2` 的 producer、`eval/check_number_defects.FOOTER`、`eval/check_rounding_fidelity._TAIL_RE`、`eval/eval_ragas_vs_rubric` 的 footer strip。正式定義已收攏成 `rq.strip_evidence_tail()`，**新消費端一律用它**；既有那四份沒有動——動 RAGAS 那份會移動分數，要單獨做並重新取基準。⚠ 2026-08-27 補：**producer 那一側已經收乾淨**（`agentic_rag_v2._compose_answer_tail` 是唯一產生點，且由 `verify_answer_validators` 閘門⑬c 鎖住「產出的尾巴一定切得掉」），剩下的四份都是 **consumer** 側。
- ~~**【現生產缺陷】絕對年份 filter 會命中「標著那一年、卻不含那一年數字」的 filing**（2026-08-27，收益探針 `bh-07` 抓到，回頭在生產 collection 上實測複現）~~ → **2026-08-27 當天修掉**，見 [`CHANGELOG.md`](CHANGELOG.md) 同日。
  修法是 `rq.tier1_hit_is_qualified`：**label-year 那半個 OR 只能放寬命中、不能自己構成命中**。依據是實測 77 份 filing（`multiyear`，`mdna` 的超集）——**10-K 的 `fiscal_year` 與 `report_label_year` 永遠相等（21/21），會分歧的只有 10-Q**（15/56），所以那半個 OR 的全部效果就是放行「曆年標籤符合、財年不符」的季報。生產上的作用對象只有兩組 `(ticker, year)`（`MSFT 2025`／`NVDA 2025`），blast radius 可枚舉。
...
- **【多年語料 ④】`AWS` 沒有被解析成 AMZN**（2026-08-27，收益探針 `bh-14` 抓到）。「AWS 在 2022 年的淨銷售額」抽不到 ticker → Tier 3 退回無 filter → top-5 是 META／MSFT／TSLA 的 IncomeStatement 跨公司污染。**這是實體解析不是期別問題**，與上一條（絕對年份 filter）修在不同地方，別併成一個「多年語料的損害」數字。⚠ 修法**不要走硬編碼別名表**（`AWS→AMZN`、`Azure→MSFT`…那是 O(n) 的開始，見 CLAUDE.md〈LLM 與 Python 的分工〉）；ticker 抽取本來就在 LLM 那一側，該補的是 prompt 裡「產品／子公司名也要映回母公司 ticker」這條規則。
...
| **期別探針的 gold 不再細修**（2026-08-19） | 人工複審 49 題：gold 指稱沒被新資料破壞，但 **46/49 的 gold 就是該公司最新那一份**，而 B 的規則正是「留最新」→ `gold@k` 結構上偏袒 B；3 題「gold 不是最新」的陰性對照經人工讀原文**全是假警報**（`mix-07`／`lex-12` 的更新一季逐字有同一句、`mix-10` 被路由到 news）。要修就得**重寫 gold 到 chunk 粒度並標註「哪些期別同樣可接受」**，那是重做題庫等級的工，而這條路的決定（B 開、A 關）不會因此改變。詳見 [`docs/EVAL.md`](docs/EVAL.md)〈期別探針的 gold 人工複審〉 | **2026-08-27 復活條件觸發（升生產），只做到一半，如實記在這裡**：~~同一次要把 `sem-03`／`sem-04`／`col-02`／`col-03` 的萬用字元釘死~~ **已做**（釘成單年 KB 裡實際含答案的那幾份，與 `period_probe_baseline.json` 同一條規則）；**「重寫 gold 到 chunk 粒度並標註哪些期別同樣可接受」沒做**——那是重做題庫等級的工，而升生產的決定不靠它（驗收用的是零噪音的 `check_number_defects` 與收益／損害兩支探針，不是 `gold@k`）。⚠ **但偏袒現在確實跑在生產 collection 上了**：新的復活條件是「下一次要用 `gold@k` 的差值做決定時」——那時它不再是背景說明，是會左右結論的偏差 |
