...
- ~~**「檢索已經到頂」靠的是一把作廢的尺**~~ → **2026-09-08 量到了，見 CHANGELOG。**
  **現況（2026-09-11 重量，RRF=30 ＋ 修過的 gold）：`gold@5 0.805`／`gold@20 0.902`／8/41 題（19.5%）有檢索層缺陷**（4 召回＋4 排序）。
  09-08 的頭條是 `0.683`／`0.780`／32%，差距拆帳：**2 題真的修好**（RRF 20→30）、**3 題是量尺錯**（EPS 族 gold 太窄）。
...
- **（已完成）「檢索已經到頂」這個結論靠的是一把已經作廢的尺。**
  ...
  **✅ 2026-09-10 重量並改掉了**：20 → **30**（救回 3 題、退步 0 題、+46% 檢索時間），
  證據表寫在 `rag_query.py` 該常數上方。⇒ **`@20` 從此不再等於整個候選池**，
  判讀舊數字時要記得它們是 20 時代量的。
...
## 已接受的極限：不做「內容天花板旗標」（2026-09-11 查清後決定）

**原本的計畫**：`forced_pass` 的子問題有 8/10 在 3 倍寬的檢索下 Grader 仍判不足
⇒「KB 內容裡真的沒有」是一種**沒有旗標**的天花板，而 `probe_kb_content_ceiling` 量到
陰性對照的內容誤殺是 **0/15** ⇒ 看起來可以放心做。

**推翻它的證據（E2，2026-09-11）**：逐筆去讀 R3 的 `forced_pass_detail`，並**直接讀 Qdrant
驗證**（不採信答案自己的引用宣稱）：

| 子問題 | Grader 說缺什麼 | 實情 |
|---|---|---|
| `lex-04` | 「CUDA 開發平台的推出年份（具體數字）」 | `NVDA_10K_2026.html#20` **逐字寫著**「With our introduction of CUDA in 2006」，`NVDA_10K_2025.html#19` 也有；R3 的答案引用它答對了 |
| `mh-01` | 「Alphabet (GOOGL) TTM revenue is not present」 | `GOOGL_Fundamentals_20260612.txt#0` 寫著 `Revenue (TTM) : $422.50B` |

⇒ **8 個 forced_pass 子問題裡有 2 個是 Grader 判錯，事實就在 KB 裡。誤殺率至少 2/8 ＝ 25%。**

⚠ **那個 0/15 為什麼抓不到**：`probe_kb_content_ceiling` 的陰性對照抽樣自**生產判夠**的
子問題，而危險的形狀正好是「生產判**不**夠、但事實在 KB 裡」——**取樣框架結構上排除了它**。
這是本 repo 記過多次的「量尺自備輸入」的另一種形狀：陰性對照的來源決定了它能看到什麼。

⚠ **代價不對稱而且方向是反的**：那個旗標若在 `lex-04` 上亮，會把一個**逐字有據的正確答案
變成拒答** ⇒ 比現況更糟。現況（白燒幾輪然後硬答）在這兩題上**答案是對的**。

**看起來是真的內容天花板的**（仍無旗標，但不急）：`sem-14`×2（Apple 智慧型手機／平板市佔比例）、
`mix-12`×2（NVIDIA 中國資料中心晶片**出貨量**）、`col-06`（競爭車廠的自駕功能名稱）。
共同形狀是「10-K 依規定不會揭露的東西」。

**復活條件**：① 有一支能**分辨「Grader 判錯」與「KB 真的沒有」**的量尺——關鍵是陰性對照
要能取樣到「生產判不夠但事實在 KB 裡」的子問題（例如拿本次這 2 題當種子）；
② 在那把尺上量到的誤殺率仍然很低。**在那之前不要做這個旗標。**

---

## 已知缺陷（查清楚了、還沒修）

- **`*_IncomeStatement_*.txt` 的年份標記只在 `#0`，`#1..#N` 完全沒有**（2026-09-11 查到，
  目前**無害但有潛在答錯路徑**）。那些檔是扁平的 key-value dump，切成 ~385 字元一塊，
  而 `--- Fiscal Year: YYYY ---` 只出現在第一塊。實例：
  · `NVDA_IncomeStatement#1` 有 `Diluted EPS: $4.90`（FY2026），**整塊沒有年份**；
  · `META_IncomeStatement#1` 是 `Diluted EPS: $23.49`（FY2025）、`#6` 是 `$23.86`（FY2024），
    **兩塊都沒有年份** ⇒ 撈到哪一塊就答哪一年，而且無從分辨。
  · `META_IncomeStatement#4` 的塊**中間**才出現 `--- Fiscal Year: 2024 ---` ⇒ 單一 chunk 橫跨兩個財年。
  **為什麼目前無害**：`lex-03`／`lex-07`／`lex-14` 三題都走 10-K 的損益表答對了
  （見〈已接受的極限〉上方那條的 gold 修正）。
  **修法方向**：ingest 時把財年標頭（與 ticker 一樣）**接地到每一塊**，並把財年當硬邊界
  ——這正是 filing 那條路的「期間章節」層在做的事，只是沒有套用到這些生成的 `.txt`。
  ⚠ **要重跑 ingest**（三小時＋三道閘門），所以**等下次有正當理由重建時一起做**，
  同 `MSFT_10K_2024.html#158` 那條的處理方式。
  ⚠ **在那之前不要為它加檢索層的補丁**：那會變成為一種檔案型別寫特例。
...
`forced_pass` 那 10 題裡只有 `lex-04` 是檢索問題。**不要把 32%（現在是 19.5%）當成可回收的收益。**
...
2. ~~**5/9 是名額問題，而且 `FETCH_N` 比 `RRF_TOP_N_PRIMARY` 更值得動**（3 題 vs 2 題）。~~
   **2026-09-10 sweep 實測推翻了後半句（前半句仍成立）。**

   | override | `col-01` | `lex-04` | `sem-01` | `lex-17` | `sem-03` |
   |---|---|---|---|---|---|
   | baseline | N/N | N/N | N/N | N/N | N/N |
   | `FETCH_N=300` | N/N | N/N | N/N | N/N | N/N |
   | `RRF_TOP_N_PRIMARY=50` | N/N | N/N | N/N | **@5=Y**(r3) | **@20=Y**(r9) |
   | 兩者一起 | N/N | N/N | N/N | @5=Y(r3) | @20=Y(r**10**) |

   ⚠ **`FETCH_N` 對 `rq.retrieve()` 的回傳零效果**（池大小逐字相同）⇒ 進〈已試無效總表〉。
     成因是**串聯**：prefetch 撈得再深，中間仍卡著 `RRF_TOP_N_PRIMARY=20`。
     ⇒ **T1 的歸因沒有錯，錯的是我把③格讀成一個可以單獨調的旋鈕**——
     它量的是 **prefetch 候選集**，而「進得了 prefetch」≠「擠得進 RRF 前 20」。
   ⚠ 「兩個常數要分開 sweep」對**歸因**仍然成立，對**修法**是錯的：它們在這條鏈上串聯。
   ⚠ `RRF=50` 精準救回②格那兩題；**但還不能動那個常數**——`rag_query.py` 的
     `RRF_TOP_N_PRIMARY` 註解寫著「sweep 顯示 **20 > 40**（reranker 信噪比最佳）」，
     放寬名額會讓更多候選進 rerank、**可能把別題的 gold 擠出 @5**。
     只看這 5 題會系統性高估收益 ⇒ **全量 41 題 × 3 輪 × 兩臂**（09-10 跑，結果見下）。
   ⚠ 仍然：**「進得了候選池」不等於「答案會變好」**，這裡一個字都不宣稱。

#### ✅ E1（2026-09-11，零成本）：那 3 題在 agentic 上本來就沒有損失

拿 R3 結果檔逐題查「被 sweep 救回的三題，在 RRF=20 時答案有沒有錯」：

| 題 | gold | R3 漏斗判定 | 答案 |
|---|---|---|---|
| `lex-17` | `MSFT_Fundamentals#0`（18.30） | `used_via_backstop` | 已答對——**ratio 保底早就撈回來了** |
| `mix-14` | `TSLA_10Q_202606#14/#67`（4,219） | `used` | 已答對「增加約 42.2 億美元」 |
| `sem-03` | `MSFT_10K_2026#128/#200` | `dropped_by_grader` | gold **在 `retrieved_union` 裡**卻被圈選丟掉 ⇒ 卡的是**別的層**，放寬名額無效 |

⇒ **這次 sweep 的 +0.049 `gold@5` 在 agentic 上找不到通往答案改善的路徑。**

⚠ **但這不表示該把常數改回去**，兩條路的形狀不同：
  · **agentic**：每題拆多個子問題各跑一次 `retrieve()` 再聯集 ⇒ 單次檢索的 top-5 不是瓶頸，
    收益被其他機制（保底、聯集）吸收掉。
  · **單管線**（`api_server` → web UI ＝ **使用者實際在用的**）：一次檢索，
    **top-5 就是 Generator 看到的全部** ⇒ `gold@5` 的改善直接落在這條路上。
⚠ **所以現況是一個已知的取捨**：+46% 的成本兩條路都付，收益只在使用者那條路上，
  **而那條路的答案層從來沒有任何 eval 涵蓋**。已記進 CLAUDE.md 的〈已知帶著上線的取捨〉。

#### ✅ 全量 sweep 跑完了（2026-09-10，41 題 × 3 輪 × 三臂）

`experiments/cgr_rrf_baseline20_20260910.json`／`cgr_rrf30_20260910.json`／
`cgr_rrf50_20260910.json`。**基準臂逐字重現 09-08 的數字**（`gold@5 0.683`／
`gold@20 0.780`、同樣的 4 題排序＋9 題召回）⇒ 差異可以乾淨地歸給那個常數。

| RRF | 秒/次 | 相對 20 | `gold@5` | `gold@20` | 召回問題 |
|---|---|---|---|---|---|
| **20**（現行） | 28.8 | — | 0.683 | 0.780 | 9 題 |
| **30** | 41.9 | **+46%** | **0.732** | **0.829** | 7 題 |
| 50 | 67.8 | +135% | 0.732 | 0.829 | 7 題 |

逐題（三臂 @5 的 k/3）：`lex-17` **0/3/3**、`mix-14` **0/3/3**、`sem-03` 0/0/0
（但 @20 是 0/3/3）。**其餘 38 題三臂逐字相同 ⇒ 改善 3 題、退步 0 題、
兩臂跨輪不穩定各 0 題。**

- **`mix-14` 是意外收穫**：它原本是**排序**問題（@20 進得來、@5 進不來），
  放寬融合名額後 rerank 從更大的候選集裡挑，把它推進前 5。
- **50 相對 30 純浪費**（進〈已試無效總表〉）：`RERANK_INPUT_N=50` 是重排輸入上限，
  RRF 回 50 就是 50 個全進重排，而第 31~50 名**沒有一個含 gold**。

⚠ **`rag_query.py` 的 `RRF_TOP_N_PRIMARY` 註解與這次量測衝突**：它寫著
「sweep 顯示 20 > 40（reranker 信噪比最佳）」，而 30/50 在每一格都不輸 20。
那句是在**舊 collection** 上量的，照本 repo 規則已作廢——**但不要偷偷改掉註解再換常數**，
那會讓舊結論與碼上的值對不上。要動就在同一次把註解改成
「20 vs 30 vs 50 於 2026-09-10 在 multiyear 上重量」並附證據檔。

⚠ **決定還沒下，因為代價這一邊沒有量尺**：
  · 收益 ＝ 41 題裡 **2 題**進 @5（`chunk_gold` 是聯集、偏大 ⇒ **上界**），
    而且量的是**檢索不是答案**——「修了答案會不會變好」仍然沒有量尺，
    而 09-10 的三段漏斗才剛顯示 `forced_pass` 那些題多半**不是**檢索問題。
  · 代價 ＝ 單管線（`api_server` → 使用者實際等的那條）每次查詢 **28.8s → 41.9s**；
    agentic **每個子問題各跑一次完整 `rq.retrieve()`**，所以會被子問題數乘上去
    （R3 實測 65 題／107 個子問題／平均 161s 一題）。
  ⇒ **這是產品取捨不是分數取捨**，要由人決定。**不自行更動生產常數。**
...
⚠ ~~**降級的「成因」目前無處可查**~~ → **2026-09-09 做了**（`degraded_reason`，閘門㉒），
  **2026-09-11 第一次真的用上**：R4 有 10 題降級，成因逐筆是
  `NotFoundError: Error code: 404 @ _base_client.py:1148 in request`，連續落在
  `sem-09`~`mix-04`（中間只有 `mix-03` 倖存）⇒ **provider 端的一段暫時性 404 視窗**，
  當場另外打一次 `DEFAULT_MODEL` 證實模型仍活著（3.7s 回 `OK`）。
  ⇒ **「崩潰依時間叢集不是類別」從推論變成直接證據**（R1/R2 只能靠共線性排除）。
  ⚠ **但這一格只答對了一半**：`_loc` 取的是 `extract_tb(...)[-1]` ＝ **無條件最深一層**，
  而 SDK 拋的例外最深一層住在 `openai/_base_client.py` ⇒ **它仍然沒說是哪個節點炸的**，
  而那正是閘門㉒d 宣稱要解決的事。這次真正帶資訊的是**例外型別**不是檔:行。
  **修法**：改成回報「**最深的 repo 內幀**」並保留絕對最深那一幀（兩個都印）。
  ⚠ 動它要同時改 ㉒d 的斷言，並補一條誤報對照：**整條 traceback 都在 repo 外**時
  不可以回 `"?"`（那會比現在更糟）——現況至少還印得出 SDK 的位置。
...
**目前最大的空缺**：19.5% 的可量題有檢索層缺陷（09-08 量的 32% 已重量，見上），但「修了有沒有用」沒有量尺，
