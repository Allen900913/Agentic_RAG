...
- **「最新期間」路由的覆蓋面，會隨 KB 累積年份惡化**（2026-08-14 提出）。病灶本身**已實證且已部分修好**：cross-encoder 分不出同主題 chunk 是哪一季（mix-06 probe：舊季 0.70~0.73 與最新季幾乎並列，最新季 gold 被壓到 rank 7/13/17、掉出 top-5 → context_recall=0），[`rag_query.py`](rag_query.py) `_LATEST_QUARTER_TIME_RE` 那條 deterministic 路由就是治本修法——掃 collection 解出最新 10-Q 期碼、注入 `report_period_code` 硬 filter。**但它有三個破口，且排序層完全沒有時間先驗**（[`rag_query.py:1429`](rag_query.py#L1429) 純 `raw_rerank_score` 排序）：
  ...
  **剩下的卡點**：修法要不要做、做哪一條，取決於多年語料要不要升生產（目前只是壓測，生產仍是單年的 `us_stock_rag_edgar_mdna`）。升生產前還要處理 `sem-03`／`sem-04`／`col-02`／`col-03` 的萬用字元 gold（壓測階段由探針對照凍結快照展開規避掉了）。
...
---
## 已知缺陷：ratio 機制卡在硬編碼詞表後面，口語問法整條繞過（2026-08-21）

> mi-05／lex-17 的**檢索半邊已修**（真因與修法見 [`CHANGELOG.md`](CHANGELOG.md) 2026-08-21，
> 依搬移紀律從本檔移出）。這一節記的是修完之後才**新暴露**的兩個殘留。

**殘留①：`_is_ratio_intent` 是硬編碼詞表，換個口語措辭就整條繞過。**

`agentic_rag_v2._ensure_ratio_source_coverage`（含新的確定性補撈）**整個機制**都掛在
`_RATIO_INTENT_RE` 這個正則後面。它認得「成長率／營收成長／毛利率／margin」，
**不認得口語的「成長得快不快」**——而那正是 colloquial 類題目的長相。

`col-11`（`微軟的雲端服務最近成長得快不快？`）三輪實測，成敗完全跟著 **Planner 產生的子問題措辭**走：

| run | Planner 的子問題 | `_is_ratio_intent` | 有沒有 Fundamentals | 結果 |
|---|---|---|---|---|
| `gj_mdna_65q_r1` | 微軟的雲端服務最近的**成長率**是多少 | **True** | `#1` | **PASS**（29%） |
| `gj_mdna_65q_after2` | 微軟的雲端服務最近成長得快不快？ | False | 無 | FAIL |
| `gj_variance_r2` | 微軟的雲端服務最近成長得快不快？ | False | 無 | FAIL |
| `gj_notice_e2e` | 微軟的雲端服務最近成長得快不快？ | False | 無 | FAIL |

⚠ **這不是新的退步**：三輪 FAIL 的那幾次，ratio 路徑**根本沒被執行**（意圖為假 ＝ 補撈不可達），
所以它與 2026-08-21 的修法無關。真正的訊息是**這個機制的觸發面本來就有洞，
只是先前被 Planner 偶然的改寫遮住了**。

⚠ **不要用「把『快不快』加進詞表」來修**——那正是 CLAUDE.md〈硬編碼詞表是警訊〉講的形狀，
下一個口語措辭還是會漏。判準是「這個子問題問的是不是一個有雙來源的比率指標」，
**沒有唯一機械答案 → 屬於 LLM 那一半**；正確的位置是 Planner／`parse_query_filters`
已經在跑的那次 call 上加一個附加欄位（同〈修法 A〉的做法），不要多付一次 call、
也不要改既有欄位語意（會讓 `llm_replay.py` 快取 miss）。

**復活條件／先決條件**：動它之前要先有能證偽的量尺——現在**沒有任何一支在量「ratio 意圖判對了沒有」**。
最小作法是仿 `eval/probe_realtime_need.py` 寫一支 probe（含 LLM、非閘門），
題目要**同時含口語與正式措辭**，並**保留非 ratio 的陰性對照**（否則「一律回 True」也會滿分）。

**殘留②：生成端會把 TTM 值四捨五入後與財年數字併成同一個說法。**

`lex-17` 在補撈修好之後仍然會出現：答案**引了** `MSFT_Fundamentals #0`（眼前就是 18.30%），
卻寫成「全年與最近的 **TTM** 都在約 **18%** 左右【…chunk #0】」——引用是真的、數字看起來也對，
兩個口徑就這樣消失了。四輪實測 2 次講出 18.30%、2 次四捨五入成 18%（`after2`／`r2` 各一）。

已加的護欄是 `_basis_disclosure_notice` 的新沉默條件③（**看值有沒有出現在答案裡**，
不是看有沒有引到 TTM chunk），閘門⑪c 七項 ＋ 三發變異測試（M4/M5/M6）都綠。
⚠ **但它只保證「會揭露」，不保證「模型不再四捨五入」**——那是生成端行為，
確定性 validator 管不到。要真的消掉，得動 `rq.SYSTEM_PROMPT`，
而那會動到所有題的 eval 基準，**不要為了一題去改**。
