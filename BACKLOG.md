...
- **Synthesize 要不要做成 critique↔refine 迴圈——分母 2026-09-06 做好了，還沒跑。**
...
⚠ **候選池中位數正好是 20 ＝ `RRF_TOP_N_PRIMARY` 上限**（池是滿的、gold 仍不在裡面）
⇒ 動 recall 之前，`RRF_TOP_N_PRIMARY` 的 sweep 值得先重跑一次（它上一次 sweep 在舊 collection 上，
而「20 > 40」那個結論的理由是「reranker 信噪比」——那是**排序**的理由，拿來限制**召回**是另一回事）。
